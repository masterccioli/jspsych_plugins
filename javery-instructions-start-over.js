...
jsPsych.plugins.conclude_instructions = (function() {

  var plugin = {};

  plugin.info = {
    name: 'conclude-instructions',
    description: '',
    parameters: {
      pages: {
      ...
      show_clickable_nav: {
        ...
        default: false,
        description: 'If true, then a "Start Over" and "Begin Experiment" button will be displayed beneath the instructions.'
      },
      show_page_number: {
          type: jsPsych.plugins.parameterType.BOOL,
      ...
      button_label_start_over: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'Button label start over',
        default: 'Start Over',
        description: 'The text that appears on the button to go backwards.'
      },
      button_label_begin_experiment: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'Button label begin experiment',
  ...
  plugin.trial = function(display_element, trial) {
    ...
    function btnListener(evt){
    	...
    	if(this.id === "jspsych-instructions-start-over"){
    		start_over();
    	}
    	else if(this.id === 'jspsych-instructions-begin-experiment'){
    		begin_experiment();
    	}
    }
    ...
    function show_current_page() {
      ...
      if (trial.show_clickable_nav) {
        ...
        if (trial.allow_backward) {
          ...
          nav_html += "<button id='jspsych-instructions-start-over' class='jspsych-btn' style='margin-right: 5px;' "+allowed+">&lt; "+trial.button_label_previous+"</button>";
        }
        if (trial.pages.length > 1 && trial.show_page_number) {
            nav_html += pagenum_display;
        }
        nav_html += "<button id='jspsych-instructions-begin-experiment' class='jspsych-btn'"+
        ...
        if (current_page != 0 && trial.allow_backward) {
          display_element.querySelector('#jspsych-instructions-start-over').addEventListener('click', btnListener);
        }

        display_element.querySelector('#jspsych-instructions-begin-experiment').addEventListener('click', btnListener);
      } else {
